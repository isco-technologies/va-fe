...
type Field = {
  id: string;
  label: string;
  type: "text" | "textarea" | "select" | "number" | "file" | "subtopic" | "severity";
  options?: string[];
  required?: boolean;
};

// ─── TEMPLATE ────────────────────────────────────────────────────────────────
// Only Title is required. Severity and attachments come via custom fields
// so they can appear multiple times per finding with descriptive labels.
...
const template: { fields: Field[] } = {
  fields: [
    ...
    { id: "cvssScore",      label: "CVSS Score",     type: "number" },
    {
      ...
      options: ["NETWORK", "WEB", "CLOUD", "INFRASTRUCTURE", "ACCESS CONTROL", "DATA PROTECTION", "GOVERNANCE", "ENDPOINT SECURITY"],
    },
...
const inputCls  = "w-full bg-white border border-gray-200 text-gray-900 placeholder-gray-400 px-3 py-2 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition";
const selectCls = "w-full bg-white border border-gray-200 text-gray-900 px-3 py-2 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-400 transition";
...
export default function AdminAssessmentReviewPage() {
  ...
  const [newField, setNewField] = useState<{ id: string; label: string; type: Field["type"]; required: boolean }>({
    id: "", label: "", type: "subtopic", required: false,
  });

  const requiredFields = template.fields.filter((f) => f.required).map((f) => f.id);
  ...
  const allQuestions = useMemo(() => {
    if (!domain) return [];
    const domainQuestions = domain.questions?.map((q) => ({ ...q, controlTitle: null })) || [];
    const controlQuestions = domain.controls?.flatMap((control) =>
      (control.questions || []).map((q) => ({ ...q, controlTitle: control.title }))
    ) || [];
  ...
  const removeField = (fieldId: string) => {
    ...
    setValues((prev) => { const u = { ...prev }; delete u[fieldId]; return u; });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dynamicEntries: { label: string; value: string | null; type: string; subtopic: string | null }[] = [];
      ...
      selectedFields.forEach((fieldId) => {
        ...
        if (field.type === "subtopic") {
          ...
          dynamicEntries.push({ label: field.label, value: null, type: "subtopic", subtopic: null });
          return;
        }
        ...
        if (value instanceof File) {
          ...
          dynamicEntries.push({ label: field.label, value: value.name, type: "file", subtopic: walkSubtopic });
          return;
        }

        dynamicEntries.push({ label: field.label, value: String(value), type: field.type, subtopic: walkSubtopic });
      });
  ...
  const handleAddCustomField = () => {
    ...
    const field: Field = { ...newField, id: `custom-${Date.now()}` };
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-mono text-gray-400 uppercase tracking-widest mb-1">Assessment Review</p>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{assessmentName || "Assessment"}</h1>
            {companyName && <p className="text-sm text-gray-400 mt-0.5">{companyName}</p>}
          </div>
          {!loading && domains.length > 0 && (
            <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1 shadow-sm">
              <button type="button" disabled={currentDomainIndex === 0}
              ...
              <button type="button" disabled={currentDomainIndex === domains.length - 1}
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition ${isOpen ? "bg-indigo-500" : "bg-gray-100 group-hover:bg-gray-200"}`}>
                      {isOpen ? <ChevronDown size={16} className="text-white" /> : <ChevronRight size={16} className="text-gray-500" />}
                    </div>
                    <div className="text-left">
                      <p className="text-xs text-gray-400 font-mono uppercase tracking-wider">Assessment Review</p>
                      <h3 className="font-semibold text-gray-900 text-sm mt-0.5">{domain.name}</h3>
                    </div>
                  </div>
                  <span className="text-xs text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full">{allQuestions.length}Q</span>
                </button>
                {isOpen && (
              ...
              <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Create technical findings discovered during this assessment.</p>
                  </div>
                  <button type="button" onClick={() => setShowFieldSelector((p) => !p)}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition ${
                      showFieldSelector ? "bg-indigo-500 border-indigo-400 text-white" : "border-gray-200 text-gray-500 hover:text-gray-700 hover:border-gray-300"
                    }`}
                  >
                ...
                {showFieldSelector && (
                  <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    <p className="text-xs text-gray-400 mb-3 uppercase tracking-wider font-mono">Optional Fields</p>
                        ...
                        return (
                          <button key={field.id} type="button"
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition ${
                              selected ? "bg-indigo-500 border-indigo-400 text-white" : "border-gray-200 text-gray-500 hover:border-gray-400 hover:text-gray-700 bg-white"
                            }`}
                          >
                ...
                {submitSuccess && (
                  <div className="mx-6 mt-4 flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm px-4 py-3 rounded-lg">
                    <CheckCircle2 size={15} /> Finding added successfully.
                  </div>
                )}
                ...
                {submitError && (
                  <div className="mx-6 mt-4 flex items-center justify-between gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg">
                    <div className="flex items-center gap-2"><AlertCircle size={15} />{submitError}</div>
                    <button type="button" onClick={() => setSubmitError(null)}><X size={14} /></button>
                  </div>
                )}
                      ...
                      return (
                        <div key={field.id} className="flex items-center justify-between pt-2">
                          <div className="flex items-center gap-3 flex-1">
                            <span className="text-xs font-bold uppercase tracking-widest text-indigo-600">{field.label}</span>
                            <div className="flex-1 h-px bg-indigo-200" />
                          </div>
                          <button type="button" onClick={() => removeField(field.id)} className="ml-3 text-gray-300 hover:text-red-400 transition">
                            <X size={14} />
                          </button>
                        </div>
                      );
                      ...
                      <div key={field.id} className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            {field.label}{isRequired && <span className="text-red-400 ml-1">*</span>}
                          </label>
                          {!isRequired && (
                            <button type="button" onClick={() => removeField(field.id)} className="text-gray-300 hover:text-red-400 transition">
                              <X size={14} />
                            </button>
                          )}
                        </div>
                        {field.type === "text" && (
                          <input value={values[field.id] || ""} required={field.required} className={inputCls}
                            placeholder={`Enter ${field.label.toLowerCase()}`}
                            onChange={(e) => updateValue(field.id, e.target.value)} />
                        )}
                        {field.type === "number" && (
                          <input type="number" min={0} max={10} step={0.1} value={values[field.id] || ""} className={inputCls}
                            placeholder="0.0 – 10.0" onChange={(e) => updateValue(field.id, e.target.value)} />
                        )}
                        {field.type === "textarea" && (
                          <textarea rows={3} value={values[field.id] || ""} className={`${inputCls} resize-none`}
                            placeholder={`Enter ${field.label.toLowerCase()}`}
                            onChange={(e) => updateValue(field.id, e.target.value)} />
                        )}
                        {field.type === "select" && (
                          <select value={values[field.id] || ""} required={field.required} className={selectCls}
                            onChange={(e) => updateValue(field.id, e.target.value)}>
                        ...
                        {field.type === "file" && (
                          <div>
                            ...
                            {values[field.id] && (
                              <p className="text-xs text-gray-400 mt-1">Selected: {values[field.id].name}</p>
                            )}
                          </div>
                        )}
                      </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <div className="p-5 space-y-3">
              <div className="space-y-1.5">
                ...
                <input
                  placeholder={newField.type === "subtopic" ? "e.g. Network Details" : "e.g. Screenshot"}
                  className={inputCls}
                  value={newField.label}
                  onChange={(e) => {
                    const label = e.target.value;
                    setNewField((prev) => ({ ...prev, label, id: label.toLowerCase().replace(/\s+/g, "-") }));
                  }}
                />
                {newField.type === "subtopic" && (
                  <p className="text-xs text-gray-400">Appears as a section divider. No input field attached.</p>
                )}
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Type</label>
                <select className={selectCls} value={newField.type}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <div className="flex-1 bg-gray-100 rounded-full h-1">
          <div className="bg-indigo-400 h-1 rounded-full transition-all duration-300"
            style={{ width: `${((index + 1) / questions.length) * 100}%` }} />
        </div>
        <span className="text-xs text-gray-400 font-mono shrink-0">{index + 1}/{questions.length}</span>
      </div>
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 space-y-3">
        {question.controlTitle && (
          <p className="text-xs font-medium text-indigo-500 font-mono">{question.controlTitle}</p>
        )}
      ...
      <div className="flex items-center gap-2">
        <button type="button" disabled={index === 0} onClick={() => setIndex((i) => Math.max(i - 1, 0))}
        ...
        <div className="flex-1 flex justify-center gap-1">
          {questions.slice(0, Math.min(questions.length, 8)).map((_, i) => (
            <button key={i} type="button" onClick={() => setIndex(i)}
              className={`h-1.5 rounded-full transition-all ${i === index ? "w-4 bg-indigo-400" : "w-1.5 bg-gray-200 hover:bg-gray-300"}`} />
          ))}
          {questions.length > 8 && <span className="text-gray-300 text-xs self-end">…</span>}
        </div>
        <button type="button" disabled={index === questions.length - 1} onClick={() => setIndex((i) => Math.min(i + 1, questions.length - 1))}
          className="p-2 rounded-lg border border-gray-200 disabled:opacity-30 hover:bg-gray-100 transition text-gray-500">
